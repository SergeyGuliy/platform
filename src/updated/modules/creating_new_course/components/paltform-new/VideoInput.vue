    ...
    <div class="block">
      ...
      <v-button custom-type="text" custom-style="primary" v-if="formData.title === null" @click="formData.title = ''">
        <div style="display: flex; align-items: center">
          ...
          <span class="bottom__inner" style="font-weight: 600">Добавить заголовок к видео</span>
        </div>
      </v-button>
      <div v-else class="input-block" style="max-width: 50%">
...
export default {
  ...
  created() {
    ...
    if (this.crossData) {
      this.formData = this.crossData.title;
      this.data.video.src = this.crossData.video.url;
      this.data.video.id = this.crossData.video.id;
      this.data.video.name = this.crossData.video.name;
