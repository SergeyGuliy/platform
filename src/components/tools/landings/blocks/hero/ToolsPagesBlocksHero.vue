  ...
  <div>
    ...
    <v-form @submit="send" id="page-create-form">
      <template v-if="localStep === STEP_MAIN">
        ...
        <v-content-showable title="Изображение">
          <v-file name="media" rules="required" v-model="content.media" />
        </v-content-showable>

        <v-content-showable title="Подзаголовок" showable v-model="show.subtitle">
          <v-input name="subtitle" rules="required" v-model="content.subtitle" />
        </v-content-showable>
            ...
            <div class="builder-box__layouts">
              <label>
                ...
                <div class="builder-box__layouts-wrapper">
                  <v-tools-pages-blocks-hero-preview :request="request" :block="content" :layout="item.value">
...
<script>
import { PAGE_BLOCK_HERO_LAYOUTS } from "../../../../../stubs/tools-pages";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      show: {
        subtitle: content.subtitle ? true : false,
        button: content.button.text ? true : false
      },
      layouts: PAGE_BLOCK_HERO_LAYOUTS
    };
  },
  watch: {
    "show.subtitle": mapWatch("subtitle"),
