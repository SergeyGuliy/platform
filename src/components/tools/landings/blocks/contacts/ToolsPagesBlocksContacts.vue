<template>
  <div>
    ...
    <v-container-title-wrapper v-if="localStep !== STEP_CONTACTS_SETTINGS">
      <v-container-title-name>
        {{ localStep === STEP_MAIN ? "Hero" : "" }}
        {{ localStep === STEP_LAYOUT ? "Hero шаблон" : "" }}
      </v-container-title-name>
    ...
    <v-form @submit="send" id="page-create-form">
      <template v-if="localStep === STEP_MAIN">
        ...
        <v-content-showable title="Email для контакта" showable v-model="show.email">
          <v-input name="email" rules="required|email" v-model="content.email" />
        </v-content-showable>

        <v-content-showable title="Социальные сети" showable v-model="show.socials_accounts">
          <v-social-accounts
            @edit="setLocalStep(STEP_CONTACTS_SETTINGS)"
            name="socials_accounts"
            rules="required"
            v-model="content.socials_accounts"
          />
        </v-content-showable>
      </template>

      <template v-else-if="localStep === STEP_CONTACTS_SETTINGS">
        <v-social-accounts-settings v-model="content.socials_accounts" />
      </template>
            ...
            <div class="builder-box__layouts">
              <label>
                ...
                <div class="builder-box__layouts-wrapper">
                  <v-tools-pages-blocks-contacts-preview :request="request" :block="content" :layout="item.value">
...
<script>
import { PAGE_BLOCK_CONTACTS_LAYOUTS } from "../../../../../stubs/tools-pages";
import { mapWatch } from "../index";
import pagesBlock from "../../../../../mixins/pages-block";

const STEP_MAIN = "main";
const STEP_CONTACTS_SETTINGS = "button_settings";
const STEP_LAYOUT = "layout";

export default {
  name: "ToolsPagesBlocksContacts",
  mixins: [pagesBlock],
  data() {
    ...
    return {
      ...
      STEP_MAIN,
      STEP_CONTACTS_SETTINGS,
      STEP_LAYOUT,
      STEPS: [STEP_MAIN, STEP_CONTACTS_SETTINGS, STEP_LAYOUT],
      STEPS_ORDER: [STEP_MAIN, STEP_LAYOUT],
      localStep: STEP_MAIN,

      show: {
        title: content.title ? true : false,
        email: content.email ? true : false,
        socials_accounts: content.socials_accounts.length ? true : false
      },
      layouts: PAGE_BLOCK_CONTACTS_LAYOUTS
    };
  },
  watch: {
    "show.title": mapWatch("title"),
    "show.email": mapWatch("email"),
    "show.socials_accounts": mapWatch("socials_accounts", [])
  }
};
</script>
